...
public abstract class GuiLttlChart extends JPanel
{
	...
	private JDialog expandDialog;
	private GuiLttlChart expandedChart;
	private LttlTimeline chartObject;
	private boolean dragged = false;

	/**
	 * @param chartObject
	 *            this chart object will automatically update with values
	 * @param title
	 *            can be null
	 */
	public GuiLttlChart(LttlTimeline chartObject, String title)
	{
		...
		final JMenuItem expandMenuItem = new JMenuItem();
		expandMenuItem.addActionListener(new ActionListener()
		{
			@Override
			public void actionPerformed(ActionEvent e)
			{
				final GuiLttlChart original = GuiLttlChart.this;
				expandedChart = new GuiLttlChart(GuiLttlChart.this.chartObject,
						original.title)
				{
					@Override
					public void pointsChangeInternal()
					{
						original.points.clear();
						original.points.addAll(expandedChart.points);
						original.pointsChangeInternal();
					}

					@Override
					void onChange()
					{
						original.dragged = dragged;
						dragged = false;
						original.onChange();
					}
				};
				expandedChart.points.clear();
				expandedChart.points.addAll(original.points);
				expandedChart.isExpanded = true;
				expandDialog = GuiHelper.createDialog(GuiLttlChart.this.title
						+ " [EXPANDED]", 600, 400, true, true,
						ModalityType.MODELESS, expandedChart, new LttlCallback()
						{
							@Override
		...
		addMouseListener(new MouseAdapter()
		{
			...
			public void mouseReleased(MouseEvent event)
			{
				...
				{
					if (!isExpanded && expandDialog == null)
					{
					...
					for (Point point : points)
					{
						if (point.x > newPoint.x)
						{
							if (Math.abs(point.x - newPoint.x) < 0.001f)
								return;
							if (lastPoint != null
									&& Math.abs(lastPoint.x - newPoint.x) < 0.001f)
								return;
		...
		addMouseMotionListener(new MouseMotionListener()
		{
			...
			{
				if (movingIndex == -1) return;
				float nextX = movingIndex == points.size() - 1 ? maxX : points
						.get(movingIndex + 1).x - 0.001f;
				if (movingIndex == 0) nextX = 0;
				float prevX = movingIndex == 0 ? 0 : points
						.get(movingIndex - 1).x + 0.001f;
				Point point = points.get(movingIndex);
				point.x = Math.min(
						nextX,
						Math.max(prevX, (event.getX() - chartX)
								/ (float) chartWidth * maxX));
				point.y = Math.min(maxY,
						Math.max(0, chartHeight - (event.getY() - chartY))
								/ (float) chartHeight * maxY);
				pointsChangeInternal();
				dragged = true;
				repaint();
			}
			...
			public void mouseMoved(MouseEvent event)
			{
				...
				{
					int x = chartX
							+ (int) (chartWidth * (point.x / (float) maxX));
					int y = chartY + chartHeight
							- (int) (chartHeight * (point.y / (float) maxY));
					if (Math.abs(x - mouseX) <= pointSize
							&& Math.abs(y - mouseY) <= pointSize)
					{
						overIndex = i;
						break;
					}
	...
	/**
	 * called after the points have been modified on chart object, only when not dragging
	 */
	abstract void onChange();

	void pointsChangeInternal()
	{
		if (chartObject != null)
		{
			chartObject.getPoints().clear();
			chartObject.getPoints().addAll(getValues());
			chartObject.modified();
		}
	}

	public Vector2Array getValuesVector2Array()
	{
		return new Vector2Array(getValues());
	}

	public float[] getValues()
	{
		float[] values = new float[points.size() * 2];
		int i = 0;
		for (Point point : points)
		{
			values[i * 2] = point.x;
			values[i * 2 + 1] = point.y;
			i++;
		}
		return values;
	}
	...
	public void setValues(Vector2Array v)
	{
		points.clear();
		for (int i = 0, n = v.size(); i < n; i++)
		{
			points.add(new Point(v.getX(i), v.getY(i)));
		}
		if (expandedChart != null)
		{
			expandedChart.setValues(v);
		}
	}

	public void setValues(float[] v)
	{
		Lttl.Throw(v.length % 2 == 0);
		points.clear();
		for (int i = 0, n = v.length; i < n; i += 2)
		{
	...
	Point pixelToPoint(float x, float y)
	{
		...
		point.x = Math.min(maxX, Math.max(0, x - chartX) / (float) chartWidth
				* maxX);
		point.y = Math.min(maxY, Math.max(0, chartHeight - (y - chartY))
				/ (float) chartHeight * maxY);
		return point;
	}
	...
	protected void paintComponent(Graphics graphics)
	{
		...
		if (expandDialog == null)
		{
			if (numberHeight == 0)
			{
				numberHeight = getFont().layoutGlyphVector(
						g.getFontRenderContext(), new char[]
						{ '0' }, 0, 1, Font.LAYOUT_LEFT_TO_RIGHT)
			...
			maxX = 1;
			{
				...
				int xSplit = (int) Math.min(10, chartWidth
						/ (maxAxisLabelWidth * 1.5f));
			...
			maxY = 1;
			{
				int ySplit = isExpanded ? Math.min(10, chartHeight
						/ (numberHeight * 3)) : 4;
				for (int i = 0; i <= ySplit; i++)
				{
					...
					if (isExpanded)
						g.drawString(label, yAxisWidth - 6 - labelWidth, y
								+ numberHeight / 2);
					if (i != 0 && i != ySplit)
					{
						g.setColor(Color.lightGray);
			...
			for (int i = 0, n = points.size(); i < n; i++)
			{
				...
				if (isExpanded)
				{
					g.setColor(Color.black);
					x = Math.max(chartX + 2,
							Math.min(chartX + chartWidth - labelWidth, x));
					y -= 3;
					if (y < chartY + numberHeight + 3) y += 27;
					else if (n > 1)
					{
						Point comparePoint = i == n - 1 ? points.get(i - 1)
								: points.get(i + 1);
						if (y < chartY + chartHeight - 27
								&& comparePoint.y > point.y) y += 27;
					}
		...
		{
			String expandedText = "EXPANDED";
			int titleWidth = metrics.stringWidth(expandedText);
			int x = chartWidth / 2 - titleWidth / 2;
			int y = chartY + chartHeight / 2 - numberHeight / 2;
			g.setColor(Color.lightGray);
			g.drawString(expandedText, x, y + numberHeight);
		}
	}
